...
4. Update Jenkins metadata for installed-engine jobs:
   ...
   - `.jenkins/config.groovy` `kanobuildUbtArgs` when a verified compiler/toolchain pin is required
...
6. Run local validation before Jenkins:
   ...
   - for the current UE 5.8 installed engine, local validation uses `KANOBUILD_UBT_ARGS=-CompilerVersion=14.51.36231`
